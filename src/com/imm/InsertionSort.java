...
public class InsertionSort {
    ...
    //   2=j i=3                     while{ array[j+1] = array[j]             (j-- =1)j(4) > (1)current ->  (j-- =0)j(2) > (1)current  EndOfWile}           a[j]==current
    //     | |                                                   *                    |                             |                                       |
    //{2,4,8,1,3}  current(i)=1 8(j) > 1(current) ->shift {2,4,8,8,3}      ->      {2,4,8,8,3}  ->            shift{2,4,4,8,3} -->   shift{4,2,4,8,3} ->   {1,2,4,8,3}
